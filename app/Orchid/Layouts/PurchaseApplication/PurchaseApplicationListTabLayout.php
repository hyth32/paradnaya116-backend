...
class PurchaseApplicationListTabLayout extends BaseApplicationListTabLayout
{
    protected string $routeName = 'purchase-applications.index';
    ...
    protected function navigations(): iterable
    {
        return [
            Menu::make('Новые заявки')
                ...
                ->badge(fn () => RentalApplication::query()->new()->where('type', RentalApplicationType::Purchase)->count(), Color::PRIMARY),

            Menu::make('Активные заявки')
                ->route($this->routeName, ['status' => RentalApplicationStatus::Active->value])
                ->badge(fn () => RentalApplication::query()->active()->where('type', RentalApplicationType::Purchase)->count(), Color::SUCCESS),

            Menu::make('Отмененные заявки')
                ->route($this->routeName, ['status' => RentalApplicationStatus::Canceled->value])
                ->badge(fn () => RentalApplication::query()->canceled()->where('type', RentalApplicationType::Purchase)->count(), Color::SECONDARY),

            Menu::make('Выполненные заявки')
                ->route($this->routeName, ['status' => RentalApplicationStatus::Completed->value])
                ->badge(fn () => RentalApplication::query()->completed()->where('type', RentalApplicationType::Purchase)->count(), Color::INFO),
        ];
    }
}